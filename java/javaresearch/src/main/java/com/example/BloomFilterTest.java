...
import com.example.ds.BloomFilter;
...
public class BloomFilterTest {
	...
	public void testCorrectness() {
		...
		while (total < elements) {
			...
			if (total % 1000 == 0 || total == elements) {
				System.out.format("\rElements incorrectly found to be inside: %8d/%-8d (%3.2f%%)", found, total,
						100 * rate);
			}
		}
		System.out.println("\n");
	...
	public void testQuery() {
		...
		long start = bean.getCurrentThreadCpuTime();
		for (int i = 0; i < elements; i++)
			xor ^= filter.contains(prng.nextInt());
		long end = bean.getCurrentThreadCpuTime();
		long time = end - start;

		System.out.format("Queried %d elements in %d ns.\n" + "Query speed: %g elements/second\n\n", elements, time,
				elements / (time * 1e-9));
	}
	...
	public static void main(String[] args) {
		...
		if (args.length >= 1)
			test.elements = Integer.parseInt(args[0]);
		if (args.length >= 2)
			test.bitsize = Integer.parseInt(args[1]);
